...
def execute(language, username, fullname, filename, testcase, timelimit, memlimit):
    exename = filename
    global timediff
    inputfile = " <source/"+fullname+"/"+testcase + \
        ".in 1>env/output.txt 2>env/error.txt"
...
while 1:
    ...
    if(myresult != None):
        ...
        timee = str(myresult[1])
        time_limit = float(myprob[4])
        mem_limit = int(myprob[5])
        FileInDir = file_name+"_"+user_name+"_"+timee
        ...
        if(result == None):
            for x in range(int(testcase)):
                t = execute("C++", user_name, full_name, FileInDir,
                            str(x+1), time_limit, mem_limit)
